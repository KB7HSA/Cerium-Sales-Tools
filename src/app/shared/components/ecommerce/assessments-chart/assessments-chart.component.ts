...
@Component({
  selector: 'app-assessments-chart',
  imports: [NgApexchartsModule],
  templateUrl: './assessments-chart.component.html',
})
export class AssessmentsChartComponent implements AfterViewInit, OnInit {
    ...
    this.http.get<any>(`${environment.apiUrl}/dashboard/stats`).subscribe({
      next: (res) => {
        if (res.success && res.data) {
          this.updateChartData(res.data.assessmentsByDay || []);
        }
      },
      error: () => { /* silently fail */ }
    });
  ...
  private updateChartData(assessmentsByDay: { day: string; count: number }[]) {
    const days: string[] = [];
    const now = new Date();
    for (let i = 29; i >= 0; i--) {
