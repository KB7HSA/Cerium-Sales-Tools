...
-- Migration: 2026-02-21_erate_form470_user_status.sql
-- Note: Filename sorts after 2026-02-21_erate_form470.sql (creates ERateForm470).

IF EXISTS (SELECT * FROM sys.tables WHERE name = 'ERateForm470' AND schema_id = SCHEMA_ID('dbo'))
BEGIN
    ...
    IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_ERateForm470_UserStatus' AND object_id = OBJECT_ID('dbo.ERateForm470'))
    BEGIN
...
ELSE
BEGIN
    PRINT 'ERateForm470 table not found — skipping user status migration.';
END
GO
